...
/**
  * CRC16-CCITT: init=0xFFFF, poly=0x1021, MSB-first（非反射）
  * 与 imu_coordinate.md Python 示例逐 bit 等价：
  *   crc ^= byte << 8
  *   for 8 bits: if bit15 set → crc = (crc<<1)^0x1021, else crc <<= 1
  *
  * 注意：原有的 Get_CRC16_Check_Sum 使用 LSB-first 反射算法（查表法），
  * 两者算法不同，不可混用。
  */
